...
def getSentenceData(path, vocabulary_size=8000):
    ...
    # Pad to make all sequences of same length
    # max_len = max([len(x) for x in X_indices])
    # X_indices = [x + [word_to_index[unknown_token]]* (max_len - len(x)) for x in X_indices]
    # y_indices = [y + [word_to_index[unknown_token]]* (max_len - len(y)) for y in y_indices]
    
    # X_train = np.array(X_indices)
    # y_train = np.array(y_indices)

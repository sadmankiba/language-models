...
class BertSentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        bert_config = BertConfig(hidden_dropout_prob=config.hidden_dropout_prob,
        ...
        self.final_layer = torch.nn.Linear(config.hidden_size, self.num_labels)
        self.final_af = torch.nn.LogSoftmax(dim=1)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        linear_out = self.final_layer(bert_out["pooler_output"])
        # logits = self.final_af(linear_out)
        logits = linear_out
        return logits

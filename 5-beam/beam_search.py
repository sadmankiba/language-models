...
class ToyEncoderDecoderModel(nn.Module):
    ...
    def encode(self, src_input=None):
        """Create simulated encoded state"""
        enc_state_size = 10
        return torch.rand(self.batch_size, enc_state_size, self.hidden_size)
    ...
    def decode_one_step(self, t, dec_input, dec_hidden, src_hiddens, top_beamid=None):
        ...
        Args:
            dec_input: [batch_size, beam_size], the input token ids at time step t
            dec_hidden: [batch_size, beam_size, hidden_size], the hidden states at time step t
            src_hiddens: [batch_size, enc_state_size, hidden_size], the encoded source states
            top_beamid: [batch_size, beam_size], the selected beam ids at time step t-1
        """
...
def beam_search(model, src_hiddens, beam_size=5, decode_max_length=10, to_word=True):
    """
    Perform beam search given a model and encoded source states.
    """
    ...
    dec_input = torch.LongTensor(batch_size, beam_size).fill_(bos_id) # [batch_size, beam_size]
    dec_hidden = torch.zeros(batch_size, beam_size, hidden_size) # [batch_size, beam_size, hidden_size]

    # Record the cumulated per-word log-likelihood as `top_score`, and back-pointers
    ...
    for t in range(decode_max_length):
        ...
        # Add the next-word log-probability at t-th step to the cumulated top_score tensor.
...
def topK(score):
    ...
    Args: 
        score: torch.FloatTensor, [batch_size, beam_size, vocab_size]. 
               This tensor has the cumulated score of selecting the next token from a vocabulary
    ...
    top_score, top_indices = torch.topk(score, beam_size, dim=1)  # [batch_size, beam_size]
...
def select_hiddens_by_beams(hiddens, beam_id):
    ...
    Example:
        ...
        new_hiddens = torch.FloatTensor([[[0.1627, 0.4812, 0.1167, 0.4318],
                                        ...
                                        [0.9372, 0.4993, 0.5471, 0.9169]]])
    """
    batch_size, beam_size, hidden_size = hiddens.size()[:3]
    new_hiddens = torch.zeros_like(hiddens)
    for i in range(batch_size):
        new_hiddens[i] = hiddens[i].index_select(0, beam_id[i])
    return new_hiddens
...
def extract_sequences(top_score, top_wordids, top_beamids):
    ...
    for t in reversed(range(decode_max_length)):
        sequences[:, :, t] = top_wordids[t]
        if t > 0:
            top_wordids[t-1] = top_wordids[t-1].gather(1, top_beamids[t])
            top_beamids[t-1] = top_beamids[t-1].gather(1, top_beamids[t])
    
    # Sort the sequences by their final scores
    sort_score, sort_indices = top_score.sort(dim=1, descending=True)
    sequences = sequences.gather(1, sort_indices.unsqueeze(2).expand_as(sequences))

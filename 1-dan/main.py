...
def get_args():
    ...
    parser.add_argument("--emb_drop", type=float, default=0.1)
    parser.add_argument("--hid_drop", type=float, default=0.1)
    parser.add_argument("--pooling_method", type=str, default="avg", choices=["sum", "avg", "max"])
    parser.add_argument("--grad_clip", type=float, default=5.0)
    parser.add_argument("--max_train_epoch", type=int, default=15)
...
def pad_sentences(sents, pad_id):
    ...
    for i, sent in enumerate(sents):
        sents[i] += [pad_id] * (max_len - len(sent))
